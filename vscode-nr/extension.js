...
/** 弹出新版本提示 */
async function showUpdatePrompt(remoteVer) {
    ...
    if (action === '下载') {
        vscode.env.openExternal(vscode.Uri.parse('https://cjxpj.github.io/nebula-rs/'));
    }
}
...
function activate(context) {
    ...
    function updateStatusBar() {
        ...
        if (editor && editor.document.languageId === 'nr') {
            const imports = scanImports(editor.document);
            if (imports.length === 0) {
                statusBar.text = `NR v${EXT_VERSION}`;
                statusBar.tooltip = '无文件引入';
            } else {
                statusBar.text = `NR v${EXT_VERSION} +${imports.length}引入`;
                statusBar.tooltip = '已引入: ' + imports.join(', ');
            }
            statusBar.show();
        } else {
            statusBar.hide();
    ...
    const checkCmd = vscode.commands.registerCommand('nr.checkUpdate', async () => {
        ...
        if (isNewer) {
            await showUpdatePrompt(remoteVer);
        } else {
            vscode.window.showInformationMessage(
    ...
    checkUpdate().then(result => {
        if (result) {
            const [remoteVer, isNewer] = result;
            if (isNewer) { showUpdatePrompt(remoteVer); }
        }
    });
